...
import type { NamedAsyncAction, NamedAsyncActionBase, AsyncState } from "./redux.types";
 ...
 */
export function createAsyncAction<NAME: string, T>(fsa: NamedAsyncActionBase<NAME, T>): NamedAsyncAction<NAME, T> {
  const now = Date.now();
  const { meta, ...restOfAction } = fsa;
  return {
    ...restOfAction,
    meta: {
      ...meta,
 ...
 * @returns {boolean}
 */
export function shouldPerformAsyncAction(
  state: AsyncState<*>,
  fetchTimeout: number = 30000,
  staleDataTimeout: number = 86400000
): boolean {
  if (!state) {
