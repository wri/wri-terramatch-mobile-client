...
import translate, { translateLandType } from "../../../../locales";
import Validation from "../../../../utils/validation";
import { CarbonCertificationTypeReadAll } from "wri-api";
...
type Props = {|
  ...
  +landTypesState: AsyncState<CarbonCertificationTypeReadAll>,
...
export default class LandTypeScreen extends Component<Props> {
  ...
  render() {
    const isLandTypeValid = Validation.landType.validate(this.props.selectedValues);
    ...
    return (
      <ProjectFlowScreen
        ...
        keyboard
        header={translate(`${projectTypeKey}.details.landTypes`)}
        subtext={translate(`${projectTypeKey}.details.landTypesHelp`)}
        isNextButtonEnabled={isLandTypeValid}
      >
        <TouchableCheckboxes
          items={(this.props.landTypesState.data ?? []).map(item => ({
            key: item.type ?? "",
            label: translateLandType(item.type),
            image: WRICategoryImages.type[item.type]
          }))}
